...
Panel {
  ...
  readonly property string barThroughputText: root.icon + "  ↓ " + root.formatRate(root.downloadRate) + "  ↑ " + root.formatRate(root.uploadRate)
  ...
  onOpenedChanged: {
    if (opened) {
      ...
      scanRestart.stop()
      // Throughput sampling continues while the panel is closed because the
      // compact bar label is live. Keep its prior sample to avoid a fake spike
      // when this popup is opened again.
      pingIface = ""
      routerPingSamples = []
      internetPingSamples = []
      routerPingLatency = -1
  ...
  // Poll details continuously: the bar exposes live throughput even while the
  // richer connection panel is closed.
  Timer {
    id: detailsPoll
    interval: 1500
    repeat: true
